...
> Cloud-Cost-Optimizer - Open source tool by AetherCodeHQ

`cli` `cloud` `cost` `golang` `optimization`

## What is it?

**Cloud Cost Optimizer** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Cloud-Cost-Optimizer.git
cd Cloud-Cost-Optimizer
go build -o Cloud-Cost-Optimizer .
```

### Run
```bash
./Cloud-Cost-Optimizer --help
```
...
```bash
# Basic usage
./Cloud-Cost-Optimizer --path ./target

# With options
./Cloud-Cost-Optimizer --path ./target --format json --output report.json

# Verbose mode
./Cloud-Cost-Optimizer --path ./target --verbose
```
...
### Basic scan
```bash
./Cloud-Cost-Optimizer --path ./my-project
```

### JSON report
```bash
./Cloud-Cost-Optimizer --path ./my-project --format json --output report.json
```
...
- name: Run Cloud Cost Optimizer
  run: ./Cloud-Cost-Optimizer --path . --format json --output report.json
```
